...
-- Variáveis exclusivas para o sistema de compra de eggs
local autoBuyEgg = false
local selectedEggType = ""

-- Dropdown para selecionar o tipo de egg
v16.Shop:AddDropdown("DropdownSelectEgg", {
    Title = "Select Egg",
    Description = "Choose the type of egg to buy",
    Values = { "Legendary", "Common", "Common 2" },
    Multi = false,
    Default = "Common"
})

-- Atualiza a seleção de egg
DropdownEgg:OnChanged(function(value)
    selectedEggType = value
    print("Egg selecionado:", selectedEggType)
end)

-- Função para obter o ID do egg com base no nome
local function getEggID(name)
    if name == "Legendary" then
        return 1
    elseif name == "Common 2" then
        return 2
...
-- Toggle exclusivo para auto buy de EGGS
v16.Shop:AddToggle("ToggleAutoBuyEgg", {
    Title = "Auto Buy Egg",
    Description = "buy the selected egg",
    Default = false,
    Callback = function(state)
        autoBuyEgg = state
        task.spawn(function()
            while autoBuyEgg do
                if selectedEggType and selectedEggType ~= "" then
                    local eggID = getEggID(selectedEggType)
                    if eggID then
                        local args = {
                            [1] = eggID
                        }
                ...
                task.wait(1) -- ajuste o delay conforme preferir
            end
        end)
    end
})
